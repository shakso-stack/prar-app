...
export default function Stage3({ job, updateJob, goToStage }) {
  ...
  function updateField(idx, field, value) {
    setArticles(prev => {
      ...
      // If journal changed, re-assign part
      if (field === "journal") {
        next[idx].part = PART_MAP[value?.trim()] || "";
      }
      if (field === "part") {
        next[idx].part = value;
      }
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "#7a6a5a", fontSize: 14, margin: 0 }}>
            Articles are sorted by journal and assigned to Parts. Edit any cell or reassign an article's Part.
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={addArticle} style={btnOutline}>+ Add Article</button>
          <button onClick={() => { updateJob({ articles, stage: 4 }); goToStage(4); }}
            style={btnPrimary}>
            Proceed to Generate →
          </button>
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap" }}>
        <div style={summaryCard("#2c1810", "#d4af7a")}>
          ...
          <span style={{ fontSize: 12 }}>Total Articles</span>
        </div>
        {PARTS.slice(1).map(p => {
      ...
      {/* Tabs */}
      <div style={{ display: "flex", borderBottom: "2px solid #e0d6c8", marginBottom: 0 }}>
        {PARTS.map(p => {
      ...
      {/* Spreadsheet */}
      <div style={{ overflowX: "auto", border: "1px solid #e0d6c8", borderTop: "none", background: "#fff", borderRadius: "0 0 8px 8px" }}>
        <table style={{ borderCollapse: "collapse", width: "100%" }}>
          <thead>
            <tr style={{ background: "#2c1810" }}>
              <th style={{ ...thStyle, width: 36 }}>#</th>
              {COLS.map(c => <th key={c.key} style={{ ...thStyle, width: c.w }}>{c.label}</th>)}
            ...
            {sortedWithIdx.map((art, displayIdx) => {
              ...
              return (
                <tr key={origIdx} style={{
                  ...
                  <td style={{ ...tdStyle, color: "#aaa", fontSize: 11, textAlign: "center" }}>{displayIdx + 1}</td>
                  {COLS.map(c => (
                    <td key={c.key} style={{
                      ...tdStyle,
                      ...
                      {isEditing ? (
                        c.key === "part" ? (
                          <select value={art.part || ""} onChange={e => updateField(origIdx, "part", e.target.value)}
                            style={{ fontFamily: "Crimson Text, serif", fontSize: 12, border: "1px solid #d4af7a", borderRadius: 3, padding: "2px 4px" }}>
                            <option value="">—</option>
                        ...
                        <div style={{ fontSize: c.key === "abstract" ? 11 : 13, maxWidth: c.w, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: c.key === "abstract" ? "normal" : "nowrap" }}
                          onClick={() => { setEditingIdx(origIdx); setEditField(c.key); }}>
...
const summaryCard = (border, text, bg = "#fff") => ({
  ...
  alignItems: "center", gap: 2, color: text, fontFamily: "Crimson Text, serif",
  minWidth: 90,
});
...
const thStyle = { padding: "10px 8px", color: "#d4af7a", fontFamily: "Crimson Text, serif", fontSize: 12, fontWeight: 600, textAlign: "left", letterSpacing: 0.5, borderRight: "1px solid #4a2c1a", whiteSpace: "nowrap" };
const tdStyle = { padding: "5px 8px", verticalAlign: "top", borderRight: "1px solid #f0e8dc", fontFamily: "Crimson Text, serif", cursor: "text" };
const cellInput = { border: "1px solid #d4af7a", borderRadius: 3, padding: "3px 6px", fontFamily: "Crimson Text, serif", fontSize: 13, background: "#fffbf0", width: "100%" };
